...
// Push instead: handler runs on `queue` (NULL means the tracking thread, where blocking stalls
// sensor reads) at up to `hz` (<= 0 for every batch). handler = NULL stops delivery.
